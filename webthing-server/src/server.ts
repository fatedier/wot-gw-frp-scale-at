...
class ThingsHandler extends BaseHandler {
  ...
  ws(ws: import('ws')/*, req: express.Request*/): void {
    ...
    const list = this.things.getThings();
    ...
    ws.on('message', (msg) => {
      ...
      let thing = this.things.getThings().find(t => t.getId() === message.id)
...
class ThingHandler extends BaseHandler {
  ...
  ws(ws: import('ws'), req: express.Request): void {
    ...
    ws.on('message', (msg) => {
console.log('WS message:', JSON.stringify(msg,null,2), thing)
      let message: {
        messageType: string;
        data: Record<string, unknown>;
      };
...
export class WebThingServer {
  ...
  constructor(
    ...
    if (this.things instanceof MultipleThings) {
      ...
      this.router.ws('/', (ws) => thingsHandler.ws(ws));
